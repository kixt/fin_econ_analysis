...
fit_mix_copula <- function(u, copulas = c("frank", "t", "gumbel", "clayton"),
                           optim.method = "L-BFGS-B", vary_start = c("yes", "no"),
  ...
  umod <- switch(match.arg(vary_start),
                 "yes" = u[sample(1:nrow(u), ceiling(0.5 * nrow(u))), ], 
                 "no" = u)
  ...
  for(i in 1:nc) {
    ...
    cops[[i]] <- do.call(fitCopula, args = list(c_obj, umod, optim.method = "BFGS"))
  } 
...
fmc_error_wrap <- function(u, copulas = c("frank", "t", "gumbel", "clayton"),
  ...
  tryCatch(
    fit_mix_copula(u = u, copulas = copulas, optim.method = optim.method, 
                   no_t_mix = no_t_mix, vary_start = "yes"),
    error = function(e) e
  )
}
...
save(mc_fit, file = "./data/results/cop_mix_fit_as.RData")
...
save(mc_vola_fit, file = "./data/results/cop_mix_fit_vola_as.RData")
